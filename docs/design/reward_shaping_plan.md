...
| `turn_credit_evidence_bridge_v2` | 已完成实现、离线分析、5-step、20-step 和 50-step | 新增 `--turn-credit-policy evidence_bridge`，对 wrong-valid 的 evidence/bridge/entity search turn 加 `evidence_search_turn_bonus=0.10`，对单跳过早回答风险 final turn 加 `early_answer_turn_penalty=0.05` | 把 turn-level credit 从 query 形态升级到证据级，同时抑制 20-step 暴露出的早停单跳回答 | 历史离线分析中，5-step 的 `dev_4869`、`dev_3741`、`dev_2429`、`dev_174`、`test_7511` 关键二跳均被 evidence candidate 命中；20-step 中这些 lost case 均被 early-answer risk 命中，另命中 `dev_6133`。5-step dev EM 0.4286、format 0.9429、平均搜索 1.8143、`missing_followup_query=0`。20-step dev EM 0.4429、format 1.0000、平均搜索 1.7714、`missing_followup_query=0`，相对 v2 5-step gained 1/lost 0。按用户要求追加 50-step 后，dev EM 回落到 0.4000、format 1.0000、平均搜索 1.7429、`missing_followup_query=0`，相对 20-step gained 0/lost 3 | 当前最强 checkpoint 仍是 20-step；50-step 是负向扩展证据，不建议继续加长步数。下一步优先做 20-step 多 seed 稳定性，或补 alias-aware/answer-normalization 诊断与 bad-loop/final-answer 约束 |
| `turn_credit_final_hop_bridge_v3` | 已完成实现、local smoke、5-step Zhihu train、dev70 和 bridge150 eval | 新增 `--turn-credit-policy final_hop_bridge`；`evidence_search_turn_bonus=0.05`、`final_hop_search_turn_bonus=0.10`、`early_answer_turn_penalty=0.05`、`missing_final_hop_turn_penalty=0.08`；保持当前 KL/std 默认配置 | 把 turn credit 从 evidence bridge search 升级到 final-hop 属性 search，并惩罚缺少 final-hop 属性查询就回答的 wrong-valid final answer | 训练前 dev-5 health Zhihu success rate 1.0。5-step 训练 40 条 trajectory、5/40 correct、35/40 valid format、平均搜索 2.1750、工具事件 87/87 成功，记录 18 个 `evidence_bridge_search`、10 个 `final_hop_attribute_search`、4 个 `early_answer_missing_followup`、0 个 `missing_final_hop_attribute`。Dev70 EM 0.4286、format 0.9714、平均搜索 1.8143、Zhihu success rate 1.0，通过不明显退化门槛。Bridge150 EM macro 0.4767、correct 77/150、format 0.7600、平均搜索 3.2267、Zhihu success rate 1.0；相对 prompt-only base 的 0.4750/74/150/0.7200 为 EM/correct 小幅正向，但低于预设 format 0.9000 门槛，也弱于 evidence 20-step 的 81/150、format 0.9400 | 记录为 partial positive，不作为干净超过 base 的最终结论；不自动扩到 20-step。下一步先补 format/max-search/final-answer guard，并让 missing-final-hop detector 在真实 wrong-valid 样本上产生可解释命中 |
| `turn_credit_final_hop_bridge_v3_guard_fix` | 已完成实现、离线分析、local smoke、5-step Zhihu train 和 dev70 eval；bridge150 eval 多次工具门槛失败 | 在 v3 基础上新增 `--final-answer-guard-turn-penalty=0.06`；`missing_final_hop_attribute` 覆盖判断改为 query 显式属性词优先，date 类不再因 observation 任意年份自动视为覆盖，并补 `birthdate/deathdate` 合写词 | 修正 v3 的两个问题：format/max-search 不收束，以及 missing-final-hop penalty 在真实 bridge wrong-valid 样本中不命中 | 对历史 v3 bridge JSONL 离线分析：`missing_final_hop_penalty_records` 从 0 提升到 1，`final_answer_guard_penalty_records=34`。真实 5-step 训练 40 条 trajectory，工具事件 86/86 成功，记录 `final_answer_guard=6`、`missing_final_hop_attribute=2`、`final_hop_attribute_search=9`。Dev70 有效：EM 0.4286、format 0.9857、平均搜索 1.8000、Zhihu success rate 1.0。Bridge150 首次未通过工具门槛：Zhihu success rate 0.9855，7 个 `url_error`，参考 EM macro 0.5042、correct 79/150、format 0.7933。第二个 key bridge-5 health 被限流，success rate 0.0。原 key rerun bridge150 仍未通过工具门槛：Zhihu success rate 0.9979，`dev_7742` 1 个 `parse_error`，参考 EM macro 0.4817、correct 79/150、format 0.7867 | 训练信号和 dev70 结果有效，但没有正式 bridge 结论；所有 bridge 参考值都不能写作指标提升，需等待 success rate 1.0 的 rerun |
